...
const { client, conn } = require('../index');
...
async function set_attendance({conn}) { // atd_list에 학생 정보를 추가하여 세팅
    let sql = `select * from std_list where sid like '10%'`; // 10번대 학생만 선택
    ...
    await print_attendance({client, conn});
}
